        ...
        {grupos.map(([categoria, links]) => (
          <div key={categoria}>
            ...
            <div style={{ border: '1px solid var(--line)', borderRadius: 12, background: 'var(--card)', overflow: 'hidden' }}>
              {links.map(l => (
                <div key={l.id} className="data-row" style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '13px 20px', borderBottom: '1px solid var(--line)' }}>
                  <span style={{ display: 'flex', alignItems: 'center', gap: 12, flex: 1, minWidth: 0 }}>
                    ...
                    <span style={{ minWidth: 0 }}>
                      <span style={{ display: 'block', fontSize: 13.5, fontWeight: 600, wordBreak: 'break-word' }}>{l.titulo}</span>
                      <span style={{ display: 'block', fontSize: 12, color: 'var(--muted)', wordBreak: 'break-all' }}>{l.url}</span>
                    </span>
                  </span>
                  <div className="row-actions" style={{ display: 'flex', gap: 8, flex: 'none' }}>
                    <a href={normalizarUrl(l.url)} target="_blank" rel="noreferrer" style={{ padding: '7px 13px', border: '1px solid var(--line)', borderRadius: 7, background: 'none', fontSize: 12.5, fontWeight: 600, color: 'var(--ink)', textAlign: 'center' }}>Abrir</a>
                    {isManager && (
                      <>
...
function LinkModal({ link, onClose, onSave }: {
  ...
  return (
    <div onClick={onClose} className="modal-overlay" style={{ position: 'fixed', inset: 0, background: 'rgba(8,17,31,.5)', zIndex: 80, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 26 }}>
      <div onClick={e => e.stopPropagation()} className="modal-card" style={{ width: '100%', maxWidth: 440, background: 'var(--card)', border: '1px solid var(--line)', borderRadius: 14, padding: 26, animation: 'fadeUp .14s ease' }}>
