...
export default function Roleta() {
  ...
  const comCorretor = leads.filter(l => !!l.corretor).length;
    ...
    <div>
      <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: 20, flexWrap: 'wrap', marginBottom: 16 }}>
        ...
        {isManager && (
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            ...
            <button onClick={shuffle} style={{ padding: '11px 18px', border: '1px solid var(--line)', borderRadius: 8, background: 'var(--card)', fontSize: 13, fontWeight: 600 }}>Embaralhar roleta</button>
          </div>
        )}
      </div>
      <div className="grid2" style={{ display: 'grid', gridTemplateColumns: '1.6fr 1fr', gap: 14 }}>
        <div style={{ border: '1px solid var(--line)', borderRadius: 12, background: 'var(--card)', overflow: 'hidden' }}>
          {fila.length === 0 && <div style={{ padding: 24, fontSize: 13.5, color: 'var(--muted)' }}>Nenhum corretor na roleta ainda. Os corretores entram ao ficar "No Plantão".</div>}
          {fila.map((f, i) => {
            ...
            return (
              <div key={f.nome} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '15px 20px', borderBottom: '1px solid var(--line)', opacity: f.ativo ? 1 : 0.5, background: !isManager && f.nome === meNome ? 'var(--bg)' : 'transparent' }}>
                ...
                <span style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <span style={{ display: 'block', fontSize: 11.5, color: 'var(--muted)' }}>{f.ativo ? 'No plantão' : 'Fora da roleta'}</span>
                </span>
        ...
        <div style={{ border: '1px solid var(--line)', borderRadius: 12, background: 'var(--card)', padding: 22 }}>
          ...
          <p style={{ fontSize: 13.5, lineHeight: 1.7, color: 'var(--muted)', margin: '0 0 18px' }}>Cada lead novo cai no corretor em plantão que faz mais tempo sem receber. Quem está fora do plantão é pulado.</p>
          <div style={{ borderTop: '1px solid var(--line)', paddingTop: 16, display: 'flex', flexDirection: 'column', gap: 12 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13 }}><span style={{ color: 'var(--muted)' }}>No plantão agora</span><span style={{ fontWeight: 700 }}>{ativos}</span></div>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13 }}><span style={{ color: 'var(--muted)' }}>Leads com corretor</span><span style={{ fontWeight: 700 }}>{comCorretor}</span></div>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13 }}><span style={{ color: 'var(--muted)' }}>Leads sem corretor</span><span style={{ fontWeight: 700, color: semCorretor > 0 ? '#B7791F' : 'inherit' }}>{semCorretor}</span></div>
          </div>
        </div>
      </div>
    </div>
